...
echo -e "\e[1;33mUsing Docker Compose file: $DOCKER_COMPOSE_FILE\e[0m\n"
...
if [ "$all_ports_available" = false ]; then
    echo -e "\n  \e[1;31m! Some ports are already in use. You may need to modify $DOCKER_COMPOSE_FILE\e[0m"
...
if [ $? -ne 0 ]; then
    ...
    echo "  1. Edit $DOCKER_COMPOSE_FILE to change ports (current changes: Prometheus 19090, Grafana 13000)"
    ...
    docker-compose -f $DOCKER_COMPOSE_FILE up -d --scale worker=3
