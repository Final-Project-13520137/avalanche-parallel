...
USAGE:
    .\comprehensive-benchmark.ps1 [OPTIONS]
...
function Generate-BenchmarkResults {
    ...
    foreach ($testCase in $TestCases) {
        Write-ColorOutput "`n📊 Processing test case: $testCase" -Color $Colors.Blue
...
"@
    ...
    foreach ($testCase in $TestCases) {
        $report += "`n### $testCase`n"
        $report += "| Workers | Architecture | Throughput (TPS) | Latency (ms) | CPU (%) | Memory (MB) |`n"
        $report += "|---------|--------------|------------------|--------------|---------|-------------|`n"
        ...
        foreach ($result in $testResults) {
            ...
            $report += "| $($result.WorkerCount) | Microservices | $($micro.Throughput) | $($micro.Latency) | $($micro.CPU) | $($micro.Memory) |`n"
            $report += "| $($result.WorkerCount) | Monolith | $($mono.Throughput) | $($mono.Latency) | $($mono.CPU) | $($mono.Memory) |`n"
        }
    }
...
"@
    ...
    $report | Out-File -FilePath "$OutputPath\benchmark-report.md" -Encoding UTF8
...
'@
    ...
    try {
        ...
        Write-ColorOutput "⚠️ Failed to generate graphs. Make sure Python and matplotlib are installed." -Color $Colors.Yellow
    }
}
...
Write-ColorOutput "`n🚀 Starting Comprehensive Avalanche Benchmark Suite`n" -Color $Colors.Green
...
Write-ColorOutput "`n📊 Generating benchmark results..." -Color $Colors.Yellow
...
if ($GenerateReport) {
    Write-ColorOutput "`n📝 Generating detailed report..." -Color $Colors.Yellow
    Generate-ComparisonReport -Results $results -OutputPath $OutputDir
}

# Generate graphs if requested
if ($GenerateGraphs) {
    Write-ColorOutput "`n📈 Generating comparison graphs..." -Color $Colors.Yellow
    Generate-ComparisonGraphs -Results $results -OutputPath $OutputDir
}

# Display summary
Write-ColorOutput "`n📋 Benchmark Summary:" -Color $Colors.Cyan
...
Write-ColorOutput "`n🎉 Comprehensive benchmark completed successfully!" -Color $Colors.Green
