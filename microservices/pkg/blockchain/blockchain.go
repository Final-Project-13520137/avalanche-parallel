...
type Blockchain struct {
	...
	logger         *zap.Logger
	genesisBlock   *Block
	txPool         map[ids.ID]*common.Transaction // Pending transactions
...
func NewBlockchain(logger *zap.Logger, maxWorkers int) (*Blockchain, error) {
	...
	bc := &Blockchain{
		logger:         logger,
		txPool:         make(map[ids.ID]*common.Transaction),
...
func (bc *Blockchain) AddTransaction(tx *common.Transaction) error {
	bc.lock.Lock()
...
func (bc *Blockchain) CreateBlock(parentIDs []ids.ID, maxTxs int) (*Block, error) {
	...
	selectedTxs := make([]*common.Transaction, 0, maxTxs)
	count := 0
...
func (bc *Blockchain) GetTransaction(id ids.ID) (*common.Transaction, error) {
	bc.lock.RLock()
	defer bc.lock.RUnlock()
...
func (bc *Blockchain) createGenesisBlock() (*Block, error) {
	genesis, err := NewBlock([]ids.ID{}, make([]*common.Transaction, 0), 0)
	if err != nil {
		return nil, err
	}
