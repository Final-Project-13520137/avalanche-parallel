...
func createTestTransactions(t *testing.T, count int) []*common.Transaction {
	transactions := make([]*common.Transaction, count)
	for i := 0; i < count; i++ {
		sender := "user" + string(rune(65+i%26))
		recipient := "recipient" + string(rune(65+i%26))
		transactions[i] = common.NewTransaction(ids.GenerateTestID(), sender, recipient, uint64(100+i), uint64(i))
	}
	return transactions
}
...
func TestBlockchainForkResolution(t *testing.T) {
	...
	// Create first branch
	txA1 := common.NewTransaction(ids.GenerateTestID(), "alice", "bob", 100, 1)
	...
	txB1 := common.NewTransaction(ids.GenerateTestID(), "charlie", "dave", 200, 1)
	...
	txA2 := common.NewTransaction(ids.GenerateTestID(), "bob", "eve", 50, 1)
	...
	for i := 0; i < 3; i++ {
		tx := common.NewTransaction(ids.GenerateTestID(), "alice", "bob", uint64(100+i), uint64(2+i))
...
func TestDoubleSpendTransaction(t *testing.T) {
	...
	tx2 := common.NewTransaction(ids.GenerateTestID(), "alice", "charlie", 100, 1) // Same nonce as tx1
...
func TestParallelConsensus(t *testing.T) {
	...
	for _, tx := range transactions {
		txClone := common.NewTransaction(ids.GenerateTestID(), tx.GetSender(), tx.GetRecipient(), tx.GetAmount(), tx.GetNonce())
