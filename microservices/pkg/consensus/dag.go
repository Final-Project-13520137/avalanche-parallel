...
func NewDefaultVertexProcessor(logger *zap.Logger, maxWorkers int) *DefaultVertexProcessor {
	if maxWorkers <= 0 {
...
type ParallelDAG struct {
	...
	frontier   *set.Set[ids.ID]    // Vertices with no children
	maxWorkers int
	processor  VertexProcessor
}

// NewParallelDAG creates a new parallel DAG
func NewParallelDAG(logger *zap.Logger, maxWorkers int, processor VertexProcessor) *ParallelDAG {
	if maxWorkers <= 0 {
		maxWorkers = 4 // Default to 4 workers
	}
...
func (dag *ParallelDAG) UpdateStatus(ctx context.Context, id ids.ID, status choices.Status) error {
	...
	if children, hasChildren := dag.reversedge[id]; hasChildren {
		for _, childID := range children {
			...
			_ = childID // Avoid unused variable warning
		}
	}
