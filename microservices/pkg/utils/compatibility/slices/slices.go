...
import "github.com/Final-Project-13520137/avalanche-parallel/microservices/pkg/utils/cmp"
...
func Sort[S ~[]E, E cmp.Bounded[E]](s S) {
	n := len(s)
	// Insert sort algorithm
	for i := 1; i < n; i++ {
		for j := i; j > 0 && cmp.Compare(s[j], s[j-1]) < 0; j-- {
			s[j], s[j-1] = s[j-1], s[j]
		}
	}
}

// SortedSlicesHaveOverlap returns true if the slices share any elements
func SortedSlicesHaveOverlap[T cmp.Bounded[T]](s1, s2 []T) bool {
	i1 := 0
	i2 := 0
	for i1 < len(s1) && i2 < len(s2) {
		c := cmp.Compare(s1[i1], s2[i2])
		switch {
		case c < 0:
			i1++
		case c > 0:
...
// SortedSlicesEqual returns true if the slices are equal
func SortedSlicesEqual[T cmp.Bounded[T]](s1, s2 []T) bool {
	if len(s1) != len(s2) {
		return false
	}
	for i := range s1 {
		if cmp.Compare(s1[i], s2[i]) != 0 {
			return false
		}
	}
	return true
