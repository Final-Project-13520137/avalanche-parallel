...
func (s *SetJSON[T]) Equals(other *SetJSON[T]) bool {
	if len(s.elements) != len(other.elements) {
		return false
	}
	for elt := range s.elements {
		if _, ok := other.elements[elt]; !ok {
			return false
		}
	}
	return true
...
// UnmarshalJSON unmarshals the JSON representation of a set
func (s *SetJSON[T]) UnmarshalJSON(b []byte) error {
	str := string(b)
	if str == avajson.Null {
		return nil
	}
...
// MarshalJSON marshals a set to JSON
func (s *SetJSON[T]) MarshalJSON() ([]byte, error) {
	var (
