...
echo -e "\e[1;36mAvalanche Parallel - Go Issues Fixer\e[0m"
echo -e "\e[1;36m======================================\e[0m"
echo -e "This script fixes common Go 1.18 compatibility issues in the codebase"

# 1. Fix sorting.go bytes.Compare issue
echo -e "\n\e[1;32mStep 1: Fixing sorting.go bytes.Compare issue...\e[0m"
...
if [ -f "$SORTING_FILE" ]; then
    ...
    echo -e "  \e[1;33m! sorting.go file not found at: $SORTING_FILE\e[0m"
fi

# 2. Fix missing set package
echo -e "\n\e[1;32mStep 2: Fixing set package issues...\e[0m"

SET_DIR="default/utils/set"
SET_FILE="$SET_DIR/set.go"

if [ ! -d "$SET_DIR" ]; then
    echo -e "  \e[1;36m* Creating directory: $SET_DIR\e[0m"
    mkdir -p "$SET_DIR"
fi

if [ ! -f "$SET_FILE" ] || [ ! -s "$SET_FILE" ]; then
    echo -e "  \e[1;36m* Creating set.go file...\e[0m"
...
EOF
    ...
    echo -e "  \e[1;32m✓ set.go created successfully!\e[0m"
else
    echo -e "  \e[1;32m✓ set.go already exists\e[0m"
fi

# 3. Fix transaction.go MissingDependencies method
echo -e "\n\e[1;32mStep 3: Fixing transaction.go MissingDependencies method...\e[0m"

TRANSACTION_FILE="pkg/blockchain/transaction.go"
if [ -f "$TRANSACTION_FILE" ]; then
    echo -e "  \e[1;36m* Reading file: $TRANSACTION_FILE\e[0m"
    
    # Fix the MissingDependencies method
    echo -e "  \e[1;36m* Fixing MissingDependencies method...\e[0m"
    sed -i 's/return set\.Set\[ids\.ID\]{}, nil/return set.Empty[ids.ID](), nil/g' "$TRANSACTION_FILE"
    
    echo -e "  \e[1;32m✓ transaction.go fixed successfully!\e[0m"
else
    echo -e "  \e[1;33m! transaction.go file not found at: $TRANSACTION_FILE\e[0m"
fi

echo -e "\n\e[1;32mAll Go issues have been fixed successfully!\e[0m"
echo -e "\e[1;36mRun tests to verify: ./scripts/run_blockchain_tests.sh\e[0m" 